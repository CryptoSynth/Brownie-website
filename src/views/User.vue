<template>
  <v-container fluid>
    ...
    <v-navigation-drawer permanent :mini-variant.sync="mini" app v-model="usertool" left>
      <v-list>
      ...
      <v-divider></v-divider>
      <v-list dense nav>
        <v-list-item :to="`/user-account/${item.url}`" v-for="(item, index) in items" :key="index">
          <v-list-item-icon>
      ...
      <v-col cols="12">
        <v-sheet light>
          <v-row align="center" justify="center">
            ...
            <v-col cols="8">
              <h1 class="display-3 text-uppercase">{{current_user.account.firstName}}'s Dashboard</h1>
            </v-col>
          </v-row>
        </v-sheet>
      </v-col>
